...
use super::backend::{
    ...
    RenderBackend, RenderDeviceCaps, SpriteDrawCommand, TilemapDrawCommand, UiDrawCommand,
};
...
impl WgpuBackend {
    ...
    fn create_state(&self) -> MFResult<(WgpuState, RenderDeviceCaps)> {
        ...
        let target = create_target(&device, self.width, self.height);
    ...
    fn render_frame(&mut self) -> MFResult<()> {
            ...
            });
        {
            ...
            if let Some(buffer) = vertex_buffer.as_ref() {
                ...
                for (index, command) in self.sprites.iter().enumerate() {
                    let texture = state
                        .textures
                        .get(&command.texture_id)
...
impl RenderBackend for WgpuBackend {
    ...
    fn init(&mut self) -> MFResult<()> {
        if !self.enabled {
            ...
            return Err(render_error(message));
        }
    ...
    fn resize(&mut self, width: u32, height: u32) -> MFResult<()> {
        ...
        if let Some(state) = self.state.as_mut() {
            state.target = create_target(&state.device, self.width, self.height);
        }
        Ok(())
    }
